...
namespace Coursework_3
{
	public partial class HomForm : Form
	{
		...
		private void ImgLoader()
		{
			...
			{
				db.OpenConnection();
				command = new SqlCommand("SELECT TOP 3 Imagename, Iamgetype, Imagepath FROM Books", db.getConnection());
		...
		private void Function_Loading_Cart() {
			...
			try{
				...
				db.OpenConnection();
				command = new SqlCommand("SELECT TOP 3 id, Name, Avtar FROM Books ORDER BY id", db.getConnection());
                ...
                id_card1 = dataGridView1[0, 0].Value.ToString();
                Label_name_book_1.Text = dataGridView1[1, 0].Value.ToString();
                Label_Author_1.Text = dataGridView1[2, 0].Value.ToString();
                label3.Text = dataGridView1[0, 0].Value.ToString();
				...
				id_card3 = dataGridView1[0, 2].Value.ToString();
				Label_name_book_3.Text = dataGridView1[1, 2].Value.ToString();
				Label_Author_3.Text = dataGridView1[2, 2].Value.ToString();
				label7.Text = dataGridView1[0, 2].Value.ToString();
				...
				MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
				history.LogWriter($"{DateTime.UtcNow.AddHours(3)} | ПОПЫТКА ЗАГРУЗКИ КАРТ - {Form.ActiveForm} | {ex.Message} | НЕУДАЧНО \n");
			}
			finally { db.CloseConnection(); }
		}
		...
		private void HomForm_FormClosed(object sender, FormClosedEventArgs e) => Environment.Exit(0);
